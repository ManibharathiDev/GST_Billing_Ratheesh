			...
			$(document).ready(function(){
							...
							content += '<table class="table table-bordered customTable">';
								...
								if(data.success == 1)
								{
									...
									$.each(item, function(index, element) 
									{
										...
										var profit = parseFloat(element.profit);
										content += '<td><span><i class="fa fa-inr"></i></span><span class="alignRight">'+profit.toFixed(2)+'</span></td>';
										
										sumtotal += parseFloat(element.profit);
									});
									content += '<tr class="tableheading">';
				...
				$(document).on('click','#pdfExport',function(){
					...
					$.ajax({
						...
						success:function(html){
							window.open("exportpdf.php?flag=profitreport&query="+html,"_blank");
						}
					});
				});
				...
				$(document).on('click','#xlExport',function(){
					...
					$.ajax({
						...
						data:data,
						success:function(html){
							window.open("exportxl.php?flag=profitreport&query="+html,"_blank");
						}
					});
