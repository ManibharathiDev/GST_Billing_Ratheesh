			...
			$(document).ready(function(){
							...
							content += '<table class="table table-bordered customTable">';
								...
								if(data.success == 1)
								{
									...
									$.each(item, function(index, element) 
									{
										...
										content += '<td>'+cdate+'</td>';
										var balance = parseFloat(element.billBalance);
										content += '<td><span class="alignLeft"><i class="fa fa-inr"></i></span><span class="alignRight">'+balance.toFixed(2)+'</span></td>';
										content += '</tr>';
										sumtotal += balance;
									});
									content += '<tr class="tableheading">';
				...
				$(document).on('click','#pdfExport',function(){
					...
					$.ajax({
						...
						success:function(html){
							window.open("exportpdf.php?flag=pendingreport&query="+html,"_blank");
						}
					});
				});
				...
				$(document).on('click','#xlExport',function(){
					...
					$.ajax({
						...
						data:data,
						success:function(html){
							window.open("exportxl.php?flag=pendingreport&query="+html,"_blank");
						}
					});
