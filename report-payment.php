		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				function getreport()
				{
					...
					if(datemode == 1)
					{
						var data = {flag:flag,search:search,sdate:sdate,activepage:activepage,show:show};
					}
					else if(datemode == 2)
					{
							...
							content += '<table class="table table-bordered customTable">';
								...
								if(data.success == 1)
								{
									...
									$.each(item, function(index, element) 
									{
										...
										content += '<tr>';
										content += '<td>'+i+'</td>';
										content += '<td>'+element.billOrderNo+'</td>';
										content += '<td>'+element.clientName+'</td>';
										...
										var cdate = date.split("-").reverse().join("-");
										content += '<td>'+cdate+'</td>';
										var payment = parseFloat(element.paymentAmount);
										content += '<td><span class="alignLeft"><i class="fa fa-inr"></i></span><span class="alignRight">'+payment.toFixed(2)+'</span></td>';
										content += '</tr>';
										sumtotal += payment;
									});
									content += '<tr class="tableheading">';
									content += '<td colspan="5" class="rightAlign textBold">Total</td>';
				...
				$(document).on('click','#pdfExport',function(){
					...
					$.ajax({
						...
						success:function(html){
							window.open("exportpdf.php?flag=paymentreport&query="+html,"_blank");
						}
					});
				});
				...
				$(document).on('click','#xlExport',function(){
					...
					$.ajax({
						...
						data:data,
						success:function(html){
							window.open("exportxl.php?flag=paymentreport&query="+html,"_blank");
						}
					});
